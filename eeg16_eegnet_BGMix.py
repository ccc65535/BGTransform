...
res_file_name='./record/eeg16-eegnet-'+aug_names[0]+'-'+aug_names[1]+'-'+str(ex*4)+'times.xlsx'
...
for duration in durations:
    ...
    for fold in trials:
        ...
        all_model=EEGNet(
                nchannel, nsample, nclass,
                time_kernel=(96, (1, nsample), (1, 1)), 
                D=1,
                separa_kernel=(96, (1, 16), (1, 1)),
                dropout_rate=0.2,
                fc_norm_rate=1).to(device=device)
        ...
        save_path='./model_save/eegnet-fold-'+str(fold)+'-'+str(duration)+'s-eeg16.pth'
        tunning=True
        new_test=True
        # new_test = False
        if tunning:
        ...
        # for sub_id in [2,9,10]:
            ...
            max_acc=0
            min_loss=np.inf
            save_path='./model_save/eeg16-eegnet-'+str(duration)+'s-fold-'+str(fold)+'-sub-'+str(sub_id)+'.pth'
