...
res_file_name='./record/eegnet-P3-'+time.strftime('%m-%d-%H-%M')+'.xlsx'
res_file=pd.ExcelWriter(res_file_name)

res_file_name2='./record/P3-eegnet-f1-'+time.strftime('%m-%d-%H-%M')+'.xlsx'
...
for duration in durations:
    ...
    for k in range(kfold):
        ...
        save_path='./model_save/eegnet-fold-'+str(k)+'-'+str(duration)+'s-P3.pth'
        ...
        for sub_id in subjects:
            ...
            # ex=4
            # aug_name='FrequencyShift'

            # aug_X,aug_y=bgt.generate_data(sub_trainX,sub_trainY,aug_name,nt=[2 for i in range(ex)]+[3 for i in range(ex)])
            ...
            max_acc=0
            min_loss=np.inf
            save_path='./model_save/p3-eegnet-'+str(duration)+'s-fold-'+str(i)+'-sub-'+str(sub_id)+'.pth'
