...
# res_file_name='./record/eegnet-nak-bgt-'+time.strftime('%m-%d-%H-%M')+'.xlsx'
# res_file=pd.ExcelWriter(res_file_name)

# aug_names=['TimeReverse','SmoothTimeMask','FrequencyShift','FTSurrogate']
aug_names=['SmoothTimeMask']
# aug_names=['SignalFlip']


first_train=True
for aug_name in aug_names:

    # for i,ex in enumerate([2,4,8,16,32]):
    for i,ex in enumerate([32]):
            if i!=0:
                first_train=False

            res_file_name='./record/aug/nak-eegnet-'+aug_name+'-'+str(ex)+'times.xlsx'
            res_file=pd.ExcelWriter(res_file_name)
            for duration in durations:
                ...
                for fold,(train_ind,test_ind) in enumerate( kf.split(trials)):
                    ...
                    all_model_save_path='./model_save/nak-eegnet-fold-'+str(fold)+'-'+str(duration)+'s.pth'
                    ...
                    for sub_id in range(nsubject):
                        ...
                        aug_X,aug_y=Augmentation.augment(aug_name,ex,sub_trainX.numpy(),sub_trainY,srate)
