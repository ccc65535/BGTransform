...
res_file_name='./record/nak-shallow-'+aug_names[0]+'-'+aug_names[1]+'-'+str(ex*4)+'times.xlsx'
...
for duration in durations:
    ...
    for fold,(train_ind,test_ind) in enumerate( kf.split(trials)):
        ...
        all_model_save_path='./model_save/nak-shallow-fold-'+str(fold)+'-'+str(duration)+'s.pth'
        ...
        if first_train:
            ...
            batch_size,max_epochs,lr = 128,600,1e-3
        ...
        for sub_id in range(nsubject):
            ...
            max_acc=0
            min_loss=np.inf
            save_path='./model_save/nak-shallow-bgt-'+str(duration)+'s-fold-'+str(fold)+'-sub-'+str(sub_id)+'.pth'
