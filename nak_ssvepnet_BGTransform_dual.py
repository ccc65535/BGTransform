...
from algorithm import SSVEPNet
from algorithm.SSVEPNet import CELoss_Marginal_Smooth
...
# np.save('bench.npy',eeg_all)

# eeg_all=np.load('./bench.npy')

res_file_name='./record/ssvepnet-nak-bgtransform-'+time.strftime('%m-%d-%H-%M')+'.xlsx'
...
for duration in durations:
    ...
    for fold,(train_ind,test_ind) in enumerate( kf.split(trials)):
        ...
        ######
        batch_size,max_epochs,lr,wd = 30,500,1e-2,0.0003
        ...
        all_model = SSVEPNet.ESNet(nchannel, nsample, nclass)
        all_model = Constraint.Spectral_Normalization(all_model)
        ...
        save_path='./model_save/nak-ssvepnet-fold-'+str(fold)+'-'+str(duration)+'s.pth'

        loss_fun= CELoss_Marginal_Smooth(nclass)
        optimizer=torch.optim.Adam(all_model.parameters(), lr=lr, betas=(0.9, 0.999), weight_decay=wd)
        ...
        for sub_id in range(nsubject):
            ...
            sub_model = SSVEPNet.ESNet(nchannel, nsample, nclass)
            sub_model = Constraint.Spectral_Normalization(sub_model)
            ...
            loss_fun= CELoss_Marginal_Smooth(nclass)
            optimizer=torch.optim.Adam(sub_model.parameters(), lr=lr, betas=(0.9, 0.999), weight_decay=wd)
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_epochs * len(sub_train_data_loader),
                                                                eta_min=5e-6)
            max_acc=0
            min_loss=np.inf
            save_path='./model_save/nak-ssvepnet-'+str(duration)+'s-fold-'+str(fold)+'-sub-'+str(sub_id)+'.pth'
