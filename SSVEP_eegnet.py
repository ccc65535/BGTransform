...
res_file_name='./record/eegnet-ssvep-'+time.strftime('%m-%d-%H-%M')+'.xlsx'
...
for duration in durations:
    ...
    for fold,(train_ind,test_ind) in enumerate( kf.split(trials)):
        ...
        templates=np.mean(eeg[:,:,:,train_trials,:],axis=3)
        ...
        for epoch in range(max_epochs):
            ...
            with torch.no_grad():
                ...
                pts=f'epoch:{epoch},train loss:{total_loss/(len(train_data_loader)):.3f},valid loss:{valid_loss:.4f},valid acc:{valid_acc:.3f},test acc:{acc:.3f}'
                print(pts)
        ...
        for sub_id in subjects:
            ...
            max_acc=0
            min_loss=np.inf
            save_path='./model_save/bench-eegnet-'+str(duration)+'s-fold-'+str(fold)+'-sub-'+str(sub_id)+'.pth'
