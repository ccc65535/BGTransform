...
def tsne(X,y,X_,y_):
    ...
    return X_trans[:n_x],y,X_trans[n_x:],y_
...
aug_titles=['TimeReverse: low dispersion','SmoothTimeMask: low diversity','FrequencyShift: high dispersion','FTSurrogate: high diversity']

ex=16
for i,type in enumerate(aug_names):
    ...
    x,y,x_,y_=tsne(sub_trainX_p,sub_trainY,aug_X_p,aug_y)
    plt.title(aug_titles[i])


    ss_score = silhouette_score(x,y)
    ss_score_aug = silhouette_score(x_,y_)
    print(f"ss_scores: {ss_score:.2f},{ss_score_aug:.2f}")
    plt.xlabel(f"raw data: {ss_score:.2f}    augmentated data:{ss_score_aug:.2f}")
