...
from algorithm.ShallowConvNet import  ShallowNet
...
res_file_name='./record/P3-shallow-'+aug_names[0]+'-'+aug_names[1]+'-'+str(ex*4)+'times.xlsx'
res_file=pd.ExcelWriter(res_file_name)

res_file_name2='./record/P3-shallow-f1-'+aug_names[0]+'-'+aug_names[1]+'-'+str(ex*4)+'times.xlsx'
res_file2=pd.ExcelWriter(res_file_name2)
...
for k in range(kfold):
    ...
    all_model_save_path='./model_save/shallow-fold-'+str(k)+'-'+str(duration)+'s-P3.pth'

    if first_train:
    
        all_model=ShallowNet(nchannel, nsample, nclass,
    ...
    for sub_id in subjects:
        ...
        aug_X1,aug_y1=bgt.generate_data(sub_trainX,sub_trainY,aug_names[0],nt=[2 for i in range(int(ex))]+[3 for i in range(ex)],srate=srate)
        aug_X2,aug_y2=bgt.generate_data(sub_trainX,sub_trainY,aug_names[1],nt=[2 for i in range(int(ex))]+[3 for i in range(ex)],srate=srate)
        
        sub_trainX=torch.cat((torch.tensor(sub_trainX),torch.tensor(aug_X1),torch.tensor(aug_X2)),dim=0)
        ...
        sub_model=ShallowNet(nchannel, nsample, nclass,
                        pool_kernel=5,
                        pool_stride=5,
        ...
        max_acc=0
        min_loss=np.inf
        save_path='./model_save/p3-shallow-'+str(duration)+'s-fold-'+str(k)+'-sub-'+str(sub_id)+'.pth'
    ...
    loo_fine_tuning_f1.append(sub_f1)
...
pd.DataFrame(np.array(loo_fine_tuning_f1).T).to_excel(excel_writer=res_file2,sheet_name=str(duration), index=False, header=False)
res_file2._save()
